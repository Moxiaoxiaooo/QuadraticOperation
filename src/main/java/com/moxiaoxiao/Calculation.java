...
public class Calculation {
    ...
    Correct: rightCounts + ( for

     */
    public static String countFormulaCorrectCounts(List<String> formulaList, List<String> ansList) {
        ...
        for (String formula : formulaList) {
            try {
                String formulaResult = getResult(formula);
                if (Double.parseDouble(formulaResult) == Double.parseDouble(getResult(ansList.get(i)))) {
                    correctList.add(i);
                } else {
                    wrongList.add(i);
                }
        ...
        result.append("Correct: ").append(correctList.size()).append(correctList.toString()).append("\n")
                .append("Wrong: ").append(wrongList.size()).append(wrongList.toString());
        return result.toString();
    }


    public static String getResult(String str) throws ByZeroException, Exception {
     ...
     * @throws Exception 异常
     */
    public static double doubleCal(double a1, double a2, char operator) throws ByZeroException, Exception {
        switch (operator) {
            case '+': {
                return a1 + a2;
            }
            ...
            case '÷': {
                if (a2 == 0) {
                    throw new ByZeroException("请确保计算式除数不为0！");
                } else {
                    return a1 / a2;
                }
            }
